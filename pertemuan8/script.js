...
const products = [
    ...
    { id: 8, nama: "Meja Rias Modern", harga: 2750000, diskon: 3000000, kategori: "Kamar Tidur", stok: 0, gambar: "https://picsum.photos/id/635/400;" },
...
function renderProducts(){
    // DOM: Menyeleksi Elemen Kontainer
    ...
    products.forEach(product => {
        ...
        if(product.diskon > 0){
            ...
            priceHTML = <span class= "text-muted text-decoration-line-through small">Rp ${product.diskon.toLocalString()}</span>;
                        <span class="fw-bold text-danger">Rp $ {product.harga.toLocalString()}</span>;
        }else{
            priceHTML = <span class="fw-bold text-dark">Rp ${product.harga.toLocalString()}</span>;
        }
        // Mengecek stok produk
        if(product.stok>0){
            buttonHTML = <button class="btn btn-buy p-2" onclick="alert (Ditambahkan keranjang: $(product.nama))">Beli Sekarang</button>
        }else{
        ...
        <div class="col-12 col-md-6 col-lg-4 col-xl-3">
            <div class="card product-card shadow-sm p-2 postion-relative">
                ${badgeHTML}
                <img src="${product.gambar}">
                class="card-img-top" alt="${product.nama}"
                <div class="card-body"><small class="text-uppercase text-muted" style="font-size:0.7rem;">{$product.kategori}</small><h6 class="card-title fw-bold mb-3 mt-1">
                    ${product.nama}</h6><p class="card-text mb-3">${priceHTML}</p>
                    $buttonHTML 
                </div>
            </div>                  
        </div>
        `;
    // menyispkan HTML ke dalam container
